...
const char *compression_str(u16 compression) {
    switch (compression) {
    case PAK_CDIR_COMPRESSED_NONE:
        return "uncompressed";
    case PAK_CDIR_COMPRESSED_DEFLATE:
        return "deflate";
    default:
        return "unknown";
    }
...
void print_pak_cdir_hdr(pak_cdir_hdr *hdr) {
...
void pak_load_cdir(pak_file *pak) {
...
int find_pak_signature(pak_file *pak) {
    ...
    if (pak->stream.bs.bytes > (CDIR_HDR_SIZE + COMMENT_LEN_MAX)) {
        min_hdr_offset = pak->stream.bs.bytes - (CDIR_HDR_SIZE + COMMENT_LEN_MAX);
    }
